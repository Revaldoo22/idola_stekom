  ...
  async function handle(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      try {
        ...
        url = new URL(up.url, window.location.origin).toString();
      } catch (err) {
        toast.error(
          "Gagal upload: " + (err instanceof Error ? err.message : "")
        );
        return;
      }
...
function ParticipantSettings({
  ...
  async function savePassword() {
    ...
    try {
      try {
        await api("/api/participant/password", {
          method: "POST",
          body: JSON.stringify({ password: pw }),
        });
      } catch (err) {
        toast.error(
          "Gagal ganti password: " + (err instanceof Error ? err.message : "")
        );
        return;
      }
...
function ContentManager() {
  ...
  async function add() {
    ...
    try {
      try {
        await api("/api/participant/contents", {
          method: "POST",
          body: JSON.stringify({ kind, url: u }),
        });
      } catch (err) {
        toast.error(
          "Gagal menambah konten: " + (err instanceof Error ? err.message : "")
        );
        return;
      }
  ...
  async function remove(id: string) {
    try {
      await api(`/api/participant/contents/${id}`, { method: "DELETE" });
    } catch {
